...
import { Check, X, Clock, Calendar, AlertCircle, CheckCircle2, Plus } from "lucide-react";
...
export default function ManagerLeavesPage() {
  ...
  const fetchData = async () => {
    ...
    const [{ data: myLeaveData, error: myLeaveError }, { data: teamLeaveData, error: teamLeaveError }] = await Promise.all([
      supabase
        .from("leaves")
        .select("*")
      ...
      supabase
        ...
        .select(`
          *,
          employee:employee_id (id, name, department, role)
        `)
        .order("created_at", { ascending: false }),
    ]);
    ...
    if (!teamLeaveError) {
      const filtered = (teamLeaveData || []).filter((req: any) => {
  ...
  const updateLeaveStatus = async (id: string, status: "Approved" | "Denied") => {
    ...
    if (!req || req?.employee?.role !== "employee") {
      setMessage({ type: "error", text: "You can only process leave requests from employees." });
      return;
    }
    ...
    if (error) {
      setMessage({ type: "error", text: `Failed to ${status === "Approved" ? "approve" : "deny"} leave request` });
    } else {
      setMessage({ type: "success", text: `Leave request ${status === "Approved" ? "approved" : "denied"}` });
      fetchData();
    }
  };

  const handleApprove = async (id: string) => updateLeaveStatus(id, "Approved");
  const handleDeny = async (id: string) => updateLeaveStatus(id, "Denied");
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1">Apply for leave and manage your team’s leave requests.</p>
        </div>
        <button
